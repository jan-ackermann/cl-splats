...
#
"""Scene / dataset reader for COLMAP and NeRF-Synthetic formats."""
...
from clsplats.utils.graphics_utils import focal2fov, fov2focal, getWorld2View2
from clsplats.utils.sh_utils import SH2RGB


class BasicPointCloud(NamedTuple):
    """Simple container for a coloured point cloud."""
...
def readColmapSceneInfo(path, images, depths, eval, train_test_exp, llffhold=8):
    ...
    if eval:
        if "360" in path:
            llffhold = 8
        if llffhold:
            logger.info("LLFF hold-out: every {hold}-th camera.", hold=llffhold)
    ...
    if not os.path.exists(ply_path):
        logger.info("Converting point3d.bin to .ply (first time only).")
        try:
...
def readNerfSyntheticInfo(path, white_background, depths, eval, extension=".png"):
    ...
    if not os.path.exists(ply_path):
        num_pts = 100_000
        logger.info("Generating random point cloud ({num})...", num=num_pts)
